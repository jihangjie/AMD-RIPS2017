...
# add parent dir to path to be able to access truncate functions
...
def iterate_train(trX, teX, trY, teY, bitsize=32, savename="untitled",
    perturbation=0., dim=288):
  ...
  w_h3 = init_weights(8*4*4, (dim, 100), dtype0, perturbation)
  ...
  dw_h3 = theano.shared(floatX(np.zeros((dim, 100)), dtype0))
  ...
  hw_h3 = theano.shared(floatX(np.ones((dim, 100)), dtype0))
  ...
  with open(savefile, 'w+') as f:
    for i in range(500):
      ...
      for start in range(0, len(trX), batch_size):
        ...
        dw_c1 = truncate(dw_c1, bitsize)
        db_c1 = truncate(db_c1, bitsize)
        dw_c2 = truncate(dw_c2, bitsize)
        db_c2 = truncate(db_c2, bitsize)
        dw_h3 = truncate(dw_h3, bitsize)
        db_h3 = truncate(db_h3, bitsize)
        dw_o = truncate(dw_o, bitsize)
        db_o = truncate(db_o, bitsize)'''
